  ...
  return (
    <div className="min-h-screen text-ink">
      <div className="relative overflow-hidden bg-gradient-to-b from-[#f7fcf8] via-white to-[#eef8f1]">
        ...
        <header className="relative z-20 mx-auto max-w-7xl px-4 py-3 md:px-6 md:py-4">
          {/* Top row: brand + socials / mobile toggle */}
          ...
          {/* Second row: full-width nav — avoids colliding with the logo */}
          <nav className="mt-3 hidden w-full items-center justify-center lg:flex">
            <div className="inline-flex max-w-full flex-wrap items-center justify-center gap-0.5 rounded-full border border-primary/35 bg-white/95 px-2 py-1 text-xs font-semibold text-slate-700 shadow-sm">
              {navLinks.map((item) => (
                <Link
                  key={item.label}
                  to={item.path}
                  className={`whitespace-nowrap rounded-full px-2.5 py-1.5 transition hover:bg-primary/15 hover:text-secondary ${
              ...
              >
                ...
                {isCSDropdownOpen && (
                  <div className="absolute left-0 top-full z-50 mt-2 w-48 rounded-lg border border-primary/20 bg-white py-2 shadow-lg">
                    <Link
                      to="/consejeria-superior"
                      className="block px-4 py-2 text-sm text-slate-700 transition hover:bg-slate-50 hover:text-primary"
                      onClick={() => setIsCSDropdownOpen(false)}
                    >
                      Consejería Superior
                    </Link>
